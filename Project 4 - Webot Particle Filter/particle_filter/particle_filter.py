...
def particle_likelihood(robot_marker_list: list[MarkerMeasure], particle_marker_list: list[MarkerMeasure]) -> float:
    ...
    marker_pairs, unmatched_robot, unmatched_particle = generate_marker_pairs(robot_marker_list, particle_marker_list)

    # Step 2: Compute the likelihood using matched markers.
    for rm, pm in marker_pairs:
        likelihood = marker_likelihood(rm, pm)
        l *= likelihood

    # Step 3: incorporate the likelihood of unmatched markers.
    for r_m in unmatched_robot:
        spurious = compute_spurious_detection_rate(r_m)
        l *= (spurious)
    for p_m in unmatched_particle:
        failure = compute_detection_failure_rate(p_m)
        l *= (failure)
...
def comptue_particle_weights(particles:list[SE2], robot_marker_measures:list[MarkerMeasure], env:Environment) -> list[float]:
    ...
    ######### START STUDENT CODE #########
    for particle in particles:
        particle_measures = env.read_marker_measures(particle)
        likelihood = particle_likelihood(robot_marker_measures, particle_measures)
        particle_weights.append(likelihood)

    weights = sum(particle_weights)
    particle_weights = [weight / weights for weight in particle_weights]
