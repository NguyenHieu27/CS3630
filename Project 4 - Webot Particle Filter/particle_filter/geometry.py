...
class SE2:
    ...
    def transform_point(self, point: Point) -> Point:
        ...
        new_y = None
        ######### START STUDENT CODE #########
        homo_pt = np.array([[point.x], [point.y], [1]])
        transform_pt = np.dot(self.T, homo_pt)

        new_x = transform_pt[0, 0]
        new_y = transform_pt[1, 0]
    ...
    def compose(self, other: 'SE2') -> 'SE2':
        ...
        new_pose = None
        ######### START STUDENT CODE #########
        composed = np.dot(self.T, other.T)

        x = composed[0, 2]
        y = composed[1, 2]
        h = math.atan2(composed[1, 0], composed[0, 0])
    ...
    def inverse(self) -> 'SE2':
        ...
        new_h = None
        ######### START STUDENT CODE #########
        inverse = np.linalg.inv(self.T)

        new_x = inverse[0, 2]
        new_y = inverse[1, 2]
        new_h = math.atan2(inverse[1, 0], inverse[0, 0])
